...
def charger_haarcascade():
    global face_cascade
    haarcascade_path = r'_internal\\haarcascade_frontalface_default.xml'
    face_cascade = cv2.CascadeClassifier(haarcascade_path)
    print("Haarcascade chargé avec succès.")

def charger_filtre():
    global filter_img
    filter_path = r'_internal\\filter.png'
    if not os.path.exists(filter_path):
        raise ValueError(f"Le fichier {filter_path} n'existe pas.")
    filter_img = cv2.imread(filter_path, cv2.IMREAD_UNCHANGED)
    if filter_img is None:
        raise ValueError(f"Erreur lors du chargement de l'image {filter_path}")
